...
export default function Topbar() {
  ...
  return (
    <>
      <header className="mobile-topbar h-[68px] md:h-[72px] border-b border-slate-100 dark:border-zinc-800 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl sticky top-0 z-30 flex items-center justify-between px-4 md:px-7">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <Link to="/notifications" aria-label="Notifications" className="mobile-icon-button md:hidden relative grid h-10 w-10 place-items-center rounded-full text-slate-300">
            <Bell size={18}/><span className="notification-dot absolute right-[9px] top-[8px] h-2 w-2 rounded-full bg-fuchsia-500"/>
          </Link>
          <button aria-label="Open profile menu" onClick={()=>setMobileOpen(true)} className="md:hidden h-9 w-9 rounded-full bg-gradient-to-br from-cyan-400 to-violet-600 p-[1px]">
