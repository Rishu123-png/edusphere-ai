...
import { XAxis, YAxis, ResponsiveContainer, Tooltip, AreaChart, Area, BarChart, Bar } from 'recharts'
...
export default function ReportsPage() {
  ...
  // One-click PDF Generator for any mode
  const generatePDFReport = (type: 'Comprehensive' | 'Weekly' | 'Monthly' = 'Comprehensive') => {
    const doc = new jsPDF()
    doc.setFontSize(18)
    doc.text(`EduSphere AI Vision — ${type} School Report`, 14, 20)
    doc.setFontSize(11)
    doc.text(`School: ${school?.name || 'Intelligent Campus'} • Generated on ${new Date().toLocaleString('en-IN')}`, 14, 28)
    doc.text(`Generated By: ${profile?.displayName || profile?.email || 'Admin'}`, 14, 34)
    ...
    autoTable(doc, {
      ...
      body: [
        ...
        ['AI Health Status', 'All 7 neural modules active and verified'],
      ],
      theme: 'grid',
      headStyles: { fillColor: [79, 70, 229] }
    })

    // Weekly or Monthly table if chosen
    const tableData = type === 'Weekly' ? weeklyData.map(d => [d.date, d.present, d.absent, d.late, `${d.rate}%`])
                    : type === 'Monthly' ? monthlyData.map(d => [d.date, d.present, d.absent, 'N/A', `${d.rate}%`])
    ...
    // AI Executive Summary at the bottom
    const startY = (doc as any).lastAutoTable.finalY + 14
    doc.setFontSize(13)
    doc.text('AI Executive Summary & Recommendations:', 14, startY)
    doc.setFontSize(10)
    aiSummaryLines.forEach((line, index) => {
      doc.text(`• ${line}`, 14, startY + 8 + (index * 6))
    })
      ...
      <PageHeader 
        ...
        action={
          <div className="flex gap-2 flex-wrap">
            <Button variant="gradient" size="sm" className="rounded-full h-10 px-4 font-bold shadow" onClick={() => generatePDFReport('Comprehensive')}>
              <FileDown size={15} className="mr-1.5" /> Generate PDF Report
            </Button>
            <Button variant="outline" size="sm" className="rounded-full h-10 px-4 font-bold" onClick={generateExcelReport}>
        ...
        <TabsContent value="weekly" className="mt-4 space-y-4">
          <Card className="rounded-[26px] overflow-hidden border border-slate-200 dark:border-zinc-800 shadow-sm">
            <div className="p-4 bg-slate-50 dark:bg-zinc-900 border-b border-slate-200 dark:border-zinc-800 flex items-center justify-between">
              ...
              <Button size="sm" variant="outline" className="rounded-full h-8 text-xs font-bold" onClick={() => generatePDFReport('Weekly')}>
        ...
        <TabsContent value="monthly" className="mt-4 space-y-4">
          <Card className="rounded-[26px] overflow-hidden border border-slate-200 dark:border-zinc-800 shadow-sm">
            <div className="p-4 bg-slate-50 dark:bg-zinc-900 border-b border-slate-200 dark:border-zinc-800 flex items-center justify-between">
              ...
              <Button size="sm" variant="outline" className="rounded-full h-8 text-xs font-bold" onClick={() => generatePDFReport('Monthly')}>
                <FileDown size={13} className="mr-1"/> PDF Export
              </Button>
            </div>
            <CardContent className="p-0">
        ...
        <TabsContent value="graph" className="mt-4 space-y-4">
          <Card className="rounded-[26px] p-5 border border-slate-200 dark:border-zinc-800 space-y-4 shadow-sm">
            <div className="flex items-center justify-between flex-wrap gap-2">
              ...
              <Button size="sm" variant="gradient" className="rounded-full font-bold h-9 px-4" onClick={() => generatePDFReport('Comprehensive')}>
